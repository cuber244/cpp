...
* const int NUM_DATA = 5;
		...
		/*
		 ...
		 // 中略 
		 c[8] = 'i';
		 c[9] = '\0'; // NULL文字、ターミネータ、これがあるから「文字列」、NULL文字のためのスペースを確保しておく必要あり char str[]="abcdefghi"; // これで、解決だが、何が起こっているというと
		 char str[10] = { 'a','b','c','i','\0' }; // をすっきりさせると↑と理解する 
		
		//⽂字の配列 あるいは ⽂字列
		char str[20]; // char str[] = "NeosVR"; // もあり。こちらはサイズを自動で決めてくれる
		str[0] = 'N';
		str[1] = 'e';
		str[2] = 'o';
		str[3] = 's'; // ここまでなら単なる文字の配列
		str[4] = 'V';
		str[5] = 'R';
		str[6] = '\0'; // これを付けると 文字列 になる。終端を表す特別な値（文字）。NULL文字
		// 中身の確認
		int i = 0;
		...
		*/
		 ...
		 //ASCIIコード
		char str[] = "NeosVR";
		// char str[] = {'N','e,'o','s','V','R'}; // NULL文字なし 
		for (int i = 0; str[i] != '\0'; ++i) {
		cout << str[i] << ": " << (int)str[i] << endl;
}
		cout << str << endl; // こうすれば一度に表示、[]はいらない、なぜか？
